...
<?php 
$sqlque	= 'uid';
$sqlque .= getSearchSql('subject|tag',$keyword,'','or'); // 페이지코드와 페이지명 검색

if($_iscallpage):
$RCD = getDbArray($table['bskrbbsdata'],$sqlque,'*','uid',$orderby,$d['search']['num'.($swhere=='all'?1:2)],$p);
?>

<div id="rb-search-search-example1">
	<ol>
		<?php while($_R=db_fetch_array($RCD)):?>
		<li><a href="<?php echo $g['s']?>/?r=<?php echo $r?>&m=bskrbbs&bid=<?php echo $_R['bbsid']?>&uid=<?php echo $_R['uid']?>"><?php echo $_R['subject']?></a></li>
		<?php endwhile?>
	</ol>
</div>
...
$_ResultArray['num'][$_key] = getDbRows($table['bskrbbsdata'],$sqlque);
?>