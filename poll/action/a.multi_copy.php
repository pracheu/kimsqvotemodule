...
include_once $g['path_module'].'bskrbbs/var/var.'.$B['id'].'.php';
...
foreach ($post_members as $val)
{
	...
	{
		$CCD = getDbArray($table[$m.'comment'],"parent='bbs".$R['uid']."'",'*','uid','desc',0,0);
		//$CCD = getDbArray($table['s_comment'],"parent='".$m.$R['uid']."'",'*','uid','desc',0,0);
		while($_C=db_fetch_array($CCD))
		{
			FB::log($_C, 'cmt=');
			$comment_minuid = getDbCnt($table[$m.'comment'],'min(uid)','');
			...
			$QVAL = "'$comment_uid','".$_C['site']."','bbs".$NOWUID."','".$_C['parentmbr']."','".$_C['display']."','".$_C['hidden']."','".$_C['notice']."','".addslashes($_C['name'])."','".addslashes($_C['nic'])."',";
			...
			if ($_C['oneline'])
			{
				...
				{
					$oneline_maxuid = getDbCnt($table[$m.'oneline'],'max(uid)','');
			...
			if ($inc_upload && $_C['upload'])
			{
				...
				getDbUpdate($table[$m.'comment'],"content='".addslashes($_content)."',upload='".$tmpupload."'",'uid='.$comment_uid);
			}
		}
	}
