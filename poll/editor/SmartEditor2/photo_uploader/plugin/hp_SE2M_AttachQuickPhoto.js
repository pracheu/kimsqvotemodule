	...
	$ON_SET_PHOTO : function(aPhotoData){
		...
		try{
			...
			for(var i = 0; i <aPhotoData.length; i++){				
				...
				console.log(this.oApp.getContents());
				console.log(sContents);
			}

			//console.log(this.oApp.getContents().replace(/\s/g,"").indexOf(sContents.replace(/\s/g,"")));
			if(this.oApp.getContents().replace(/\s/g,"").indexOf(sContents.replace(/\s/g,""))<0)
