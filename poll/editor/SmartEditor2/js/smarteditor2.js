...
//alert(oTmpNode.innerHTML);
	...
	_validateURL : function(sURL){
		...
		return /^(http|https|ftp|mailto|mvup):(\/\/)?(([-가-힣]|\w)+(?:[\/\.:@]([-가-힣]|\w)+)+)\/?(.*)?\s*$/i.test(sURL);
	}
});
//}
//{
...
nhn.husky.SE_ToolbarToggler = $Class({
	...
	$init : function(oAppContainer, bUseToolbar, bUseVideoUpload){
		this._assignHTMLObjects(oAppContainer, bUseToolbar, bUseVideoUpload);
	},

	_assignHTMLObjects : function(oAppContainer, bUseToolbar, bUseVideoUpload){
		oAppContainer = $(oAppContainer) || document;
		...
		this.videoUploadArea = cssquery.getSingle(".husky_seditor_ui_video", oAppContainer);
		//설정이 없거나, 사용하겠다고 표시한 경우 block 처리
		if( typeof(bUseVideoUpload) == 'undefined' || bUseVideoUpload === true){
